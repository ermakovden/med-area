<?php

// Presentation/Files
require __DIR__ . '/../src/Presentation/File/routes/api.files.php';

// Presentation/User
require __DIR__ . '/../src/Presentation/User/routes/api.users.registration.php';
require __DIR__ . '/../src/Presentation/User/routes/api.users.auth.php';
require __DIR__ . '/../src/Presentation/User/routes/api.users.php';

// Presentation/Analys
require __DIR__ . '/../src/Presentation/Analys/routes/api.users.analysis.php';
require __DIR__ . '/../src/Presentation/Analys/routes/api.analysis.php';
